...
This is the **Antigravity fork** — adapted for inline execution without subagents.
...
- Optional domain research (performs domain research (4 dimensions))
...
- Plans within each wave run sequentially inline (no subagents on Antigravity)
...
- Creates planner + executor (skips researcher, checker, verifier by default)
...
- Saves timestamped note to `.planning/notes/` (or `~/.gemini/antigravity/notes/` globally)
...
- `quality` — Best model for all operations
- `balanced` — Best model for planning, fast model for execution (default)
- `budget` — Fast model for writing, smallest model for research/verification
- `inherit` — Use current session model for all operations
...
- Better than reinstalling manually

Usage: `/gsd-update`

**`/gsd-join-discord`**
Community links for GSD Antigravity.

- Fork repo, original GSD Discord, and upstream project links
